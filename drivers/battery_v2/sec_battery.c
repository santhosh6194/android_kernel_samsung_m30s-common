...
void sec_bat_get_ext_event(struct sec_battery_info *battery, unsigned int ext_event_val) {

	if (!battery->wc_tx_enable)
		return;

	if (ext_event_val & BATT_EXT_EVENT_CAMERA) {
	...
	if (ext_event_val & BATT_EXT_EVENT_DEX) {
...
static int sec_bat_check_mix_temp(struct sec_battery_info *battery, int input_current)
{
	if (battery->pdata->chg_temp_check_type &&
		...
		if ((!battery->mix_limit &&
			...
			if (battery->wc_tx_enable) {
				pr_info("@Tx_Mode abnormal temperature, TX mode should turn off \n", __func__);
...
int sec_bat_set_charging_current(struct sec_battery_info *battery)
{
	...
	if (battery->wc_tx_enable && battery->wc_tx_chg_limit) {
		if (input_current >= battery->pdata->wc_tx_limit_curr) {
			pr_info("@Tx_mode: %s : Chnaged Input current %d -> %d! Tx chg limit(%d)!\n",
				__func__, input_current, battery->pdata->wc_tx_limit_curr, battery->wc_tx_chg_limit);
			input_current = battery->pdata->wc_tx_limit_curr;
		}
	}
...
static void sec_bat_set_charging_status(struct sec_battery_info *battery,
	...
	case POWER_SUPPLY_STATUS_DISCHARGING:
		if ((battery->status == POWER_SUPPLY_STATUS_FULL ||
			...
			if (battery->full_by_condition) {
				pr_info("%s : Update fg scale to 101%%\n", __func__);
				value.intval = 100;
				psy_do_property(battery->pdata->fuelgauge_name, set,
						POWER_SUPPLY_PROP_CHARGE_FULL, value);
				battery->full_by_condition = false;
			} else if (battery->capacity < 100) {
				pr_info("%s : Update fg scale to 100%%\n", __func__);
				value.intval = 99;
				psy_do_property(battery->pdata->fuelgauge_name, set,
						POWER_SUPPLY_PROP_CHARGE_FULL, value);
			} else {
				pr_info("%s : skip fg scale.\n", __func__);
			}
...
static bool sec_bat_check_fullcharged(struct sec_battery_info *battery) {
	...
	case SEC_BATTERY_FULLCHARGED_ADC:
		...
		if (battery->current_adc <
			...
			battery->full_by_condition = true;
			dev_dbg(battery->dev,
				"%s: Full Check ADC (%d)\n",
				__func__,
				battery->full_check_cnt);
...
void sec_bat_get_battery_info(struct sec_battery_info *battery) {
	...
	dev_info(battery->dev,
		"%s:Vnow(%dmV),Vavg(%dmV),Inow(%dmA),Iavg(%dmA),Isysavg(%dmA),Imax(%dmA),Ichg(%dmA),SOC(%d%%),"
...
{
	if(battery->temperature >= battery->pdata->wpc_high_threshold_normal) {
	//	battery->temperature <= battery->pdata->wpc_low_threshold_normal) {
		pr_info("@Tx_Mode : %s: Battery temperature is too high. Tx mode should turn off  \n", __func__);
		/* set tx event */
		sec_bat_set_tx_event(battery, BATT_TX_EVENT_WIRELESS_TX_HIGH_TEMP, BATT_TX_EVENT_WIRELESS_TX_HIGH_TEMP);
	} else if (battery->temperature <= battery->pdata->wpc_low_threshold_normal) {
...
static void sec_bat_wireless_vout_cntl(struct sec_battery_info *battery, int vout_now)
{
	...
	pr_info("@Tx_Mode %s : set uno & mfc vout (%d -> %d)\n", __func__, battery->wc_tx_vout, vout_now);
...
static void sec_bat_siop_event_work(struct work_struct *work)
{
	...
	if (battery->siop_event & SIOP_EVENT_WPC_CALL) {
		...
		if (battery->cable_type == SEC_BATTERY_CABLE_WIRELESS_PACK) {
			pr_info("%s : Call is on during Wireless Pack\n",__func__);
			sec_bat_set_charging_current(battery);
		} else if (battery->cable_type == SEC_BATTERY_CABLE_WIRELESS_HV_PACK) {
			pr_info("%s : Call is on during Wireless HV Pack\n",__func__);
			/* chagne voltage first */
			value.intval = WIRELESS_VOUT_CALL;
			psy_do_property(battery->pdata->wireless_charger_name, set,
							POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION, value);
			/* change current */
			sec_bat_set_charging_current(battery);
		} else {
			pr_info("%s : Call is on\n",__func__);
		}	
	} else if (!(battery->siop_event & SIOP_EVENT_WPC_CALL)) {
		value.intval = 0;
		psy_do_property(battery->pdata->wireless_charger_name, set,
						POWER_SUPPLY_EXT_PROP_CALL_EVENT, value);

		if (battery->cable_type == SEC_BATTERY_CABLE_WIRELESS_PACK) {
			pr_info("%s : Call is off during Wireless Pack\n",__func__);
			sec_bat_set_charging_current(battery);
		} else if (battery->cable_type == SEC_BATTERY_CABLE_WIRELESS_HV_PACK) {
			pr_info("%s : Call is off during Wireless HV Pack\n",__func__);
			/* recover voltage first */
...
skip_current_monitor:
	...
	dev_info(battery->dev,
		 "%s: Status(%s), mode(%s), Health(%s), Cable(%s, %s, %d, %d), rp(%d), level(%d%%), lcd(%d), slate_mode(%d), store_mode(%d)"
#if defined(CONFIG_AFC_CHARGER_MODE)
		", HV(%s, %d), sleep_mode(%d)"
#endif
#if defined(CONFIG_BATTERY_AGE_FORECAST)
...
#if defined(CONFIG_WIRELESS_TX_MODE)
	if (battery->wc_tx_enable) {
		dev_info(battery->dev, "@Tx_Mode %s: Rx(%s), WC_TX_VOUT(%d), AFC_DISABLE(%d), TX_CHG_LIMIT(%d)\n",
			__func__, sec_bat_rx_type_str[battery->wc_rx_type],
			battery->wc_tx_vout, battery->afc_disable, battery->wc_tx_chg_limit);
	}
#endif
...
#if 0
	if (is_wired_type(battery->wire_status) && !is_hv_wire_type(battery->wire_status) && (battery->wire_status != SEC_BATTERY_CABLE_HV_TA_CHG_LIMIT)) {
		if (battery->uno_en)
			sec_bat_wireless_uno_cntl(battery, !battery->uno_en);
		pr_info("@Tx_Mode %s : Doesn't support Tx mode during normal charging\n", __func__);
		wake_unlock(&battery->wpc_tx_wake_lock);
		return;
	}
#endif

	if (battery->wc_rx_type == NO_DEV) {
		...
		if (battery->wire_status == SEC_BATTERY_CABLE_HV_TA_CHG_LIMIT && !battery->uno_en) {
			sec_bat_wireless_uno_cntl(battery, !battery->uno_en);
		} else if (is_hv_wire_type(battery->wire_status)) {
			pr_info("@Tx_Mode %s : Uno OFF for charging voltage change(9V -> 5V). Rx(%d)\n", __func__, battery->wc_rx_type);
		...
		if (is_hv_wire_type(battery->wire_status)) {
			...
			if (battery->wc_tx_vout != WC_TX_VOUT_5_0V)
				sec_bat_wireless_vout_cntl(battery, WC_TX_VOUT_5_0V);

			pr_info("@Tx_Mode %s : Uno vout and MFC vout set(%d)\n", __func__, battery->wc_rx_type);
		}
	} else if ((battery->wc_rx_type == SS_PHONE) || (battery->wc_rx_type == OTHER_DEV)) {
		if (is_wired_type(battery->wire_status) && !is_hv_wire_type(battery->wire_status) && (battery->wire_status != SEC_BATTERY_CABLE_HV_TA_CHG_LIMIT)) {
			battery->wc_tx_chg_limit = true;
			sec_bat_set_charging_current(battery);
		...
		} else if (battery->wire_status == SEC_BATTERY_CABLE_NONE) {
			if (!battery->uno_en)
				sec_bat_wireless_uno_cntl(battery, !battery->uno_en);

			if (battery->wc_tx_vout != WC_TX_VOUT_7_5V)
				sec_bat_wireless_vout_cntl(battery, WC_TX_VOUT_7_5V);

			battery->wc_tx_chg_limit = false;
			sec_bat_wireless_minduty_cntl(battery, battery->pdata->tx_minduty_default);
			pr_info("@Tx_Mode %s : Uno vout and MFC vout set(%d)\n", __func__, battery->wc_rx_type);

		} else if (battery->wire_status == SEC_BATTERY_CABLE_HV_TA_CHG_LIMIT) {
			pr_info("@Tx_Mode %s : Uno OFF for charging voltage change(5V -> 9V) Rx(%d)\n", __func__, battery->wc_rx_type);	
			sec_bat_wireless_uno_cntl(battery, false);
			...
			if (battery->wc_tx_vout != WC_TX_VOUT_9_0V)
				sec_bat_wireless_vout_cntl(battery, WC_TX_VOUT_9_0V);
...
static void sec_bat_cable_work(struct work_struct *work)
{
		...
		if (!is_nocharge_type(battery->wire_status)) {
			...
			if (battery->wire_status == SEC_BATTERY_CABLE_PDIC) {
				...
				if (wireless_current < wire_current) {
					current_cable_type = battery->wire_status;
					/* Turn off TX POWER_SUPPLY_EXT_PROP_WIRELESS_AUTH_ADTto charge by wired charging having more power */
	...
	if (is_nocharge_type(battery->cable_type) ||
		...
		if (battery->cable_type == SEC_BATTERY_CABLE_TA ||
			...
			((battery->wc_rx_type == SS_PHONE) || (battery->wc_rx_type == OTHER_DEV))) {
				pr_info("@Tx_Mode: %s : UNO OFF when AFC EVENT set\n", __func__);
				sec_bat_wireless_uno_cntl(battery, !battery->uno_en);
			}
		} else {
			cancel_delayed_work(&battery->afc_work);
...
#if 0
static void sec_bat_tx_chg_switch(struct sec_battery_info *battery)
{
	if (battery->wc_tx_enable) {
		pr_info("@Tx_Mode %s : Doesn't support Tx mode during non hv charging cable\n", __func__);
		/* set tx event */
		sec_bat_set_tx_event(battery, BATT_TX_EVENT_WIRELESS_TX_5VTA_ON, BATT_TX_EVENT_WIRELESS_TX_5VTA_ON);
		/* turn off tx power */
		sec_wireless_set_tx_enable(battery, false);
	}
}
...
void sec_wireless_set_tx_enable(struct sec_battery_info *battery, bool wc_tx_enable)
{
	...
	if (wc_tx_enable) {
		/* set tx event */
		sec_bat_set_tx_event(battery, BATT_TX_EVENT_WIRELESS_TX_STATUS, BATT_TX_EVENT_WIRELESS_TX_STATUS);
...
static int make_pd_list(struct sec_battery_info *battery)
{
	...
	for (i = 1; i <= battery->pdic_info.sink_status.available_pdo_num; i++)
	{
		...
		{
			if (temp_power >= selected_pdo_power && battery->pdic_info.sink_status.power_list[i].max_voltage > selected_pdo_voltage)
			{
